...
st.write("Calculate net skins, handle payouts, and manage group-specific digital scorecards.")
...
# --- SOLUTION HERE: Single Source of Truth for Player Data ---
if 'roster_df' not in st.session_state:
    initial_rows = []
    for i in range(100):  # Allocate complete buffer ceiling space upfront
        group_num = (i // 4) + 1
        initial_rows.append({
            "Player": f"Player {i+1}",
    ...
    st.session_state.roster_df = pd.DataFrame(initial_rows)

# Create a clean running view matching the user's size constraints
# Note: We do NOT filter or manipulate this data inline before feeding the editor
active_roster_slice = st.session_state.roster_df.iloc[:num_players].copy()

# Master Performance Matrix Mapping
if 'master_scores' not in st.session_state or len(st.session_state.master_scores) != num_players:
    df_rows = []
    for i, row in active_roster_slice.iterrows():
        score_row = {
            "Group": row["Group"],
            "Player": row["Player"],
        ...
        for h in range(HOLES):
            score_row[f"Hole {h+1}"] = 0
        df_rows.append(score_row)
    st.session_state.master_scores = pd.DataFrame(df_rows)
...
# --- Player Roster Setup (Stabilized Pattern) ---
st.header("👤 2. Player Roster & Handicap Settings")
st.write("Assign players to their designated Groups and choose their starting Tee Boxes from the dropdown menus below.")
...
roster_config = {
    ...
    "Course Handicap": st.column_config.NumberColumn("Handicap Index", min_value=0, max_value=54, step=1, required=True)
}

# Fix applied: Binding to a persistent 'key' keeps uncommitted edits locked in UI memory cache
edited_roster_matrix = st.data_editor(
    active_roster_slice,
    use_container_width=True,
    hide_index=True,
    column_config=roster_config,
    key="stable_roster_editor" 
)

# Apply and process updates into persistent memory structures securely
if not edited_roster_matrix.equals(active_roster_slice):
    for i, row in edited_roster_matrix.iterrows():
        st.session_state.roster_df.at[i, "Player"] = row["Player"]
        st.session_state.roster_df.at[i, "Group"] = row["Group"]
        st.session_state.roster_df.at[i, "Tee"] = row["Tee"]
        st.session_state.roster_df.at[i, "Course Handicap"] = row["Course Handicap"]

    # Re-align dependent master scoring array profiles
    for i, row in edited_roster_matrix.iterrows():
        st.session_state.master_scores.at[i, "Player"] = row["Player"]
        st.session_state.master_scores.at[i, "Group"] = row["Group"]
        st.session_state.master_scores.at[i, "Handicap"] = row["Course Handicap"]
        st.session_state.master_scores.at[i, "Tee"] = row["Tee"]
    st.rerun()
...
group_mask = st.session_state.master_scores["Group"] == f"Group {active_g}"
active_group_df = st.session_state.master_scores[group_mask]
...
if active_group_df.empty:
    ...
    with st.form(key=f"score_form_group_{active_g}", clear_on_submit=False):
        ...
        submit_button = st.form_submit_button(label="💾 Save Hole Scores")

    if submit_button:
        for idx, row in st.session_state.master_scores.iterrows():
            if row["Group"] == f"Group {active_g}" and row["Player"] in input_scores:
                st.session_state.master_scores.at[idx, selected_hole] = input_scores[row["Player"]]
...
with st.expander(f"👀 View Group {active_g} Live Running Scorecard Summary"):
    st.dataframe(
        st.session_state.master_scores[st.session_state.master_scores["Group"] == f"Group {active_g}"],
        use_container_width=True,
        hide_index=True
    )
...
for g in range(MAX_GROUPS):
    ...
    group_data = st.session_state.master_scores[st.session_state.master_scores["Group"] == group_label].copy()
...
with tabs[-1]:
    ...
    if all_group_summaries:
        ...
        master_board = master_board[master_board['Player'].isin(st.session_state.master_scores['Player'])]
        st.dataframe(
